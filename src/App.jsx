...
export default function App() {
  ...
  const handleSendChatMessage = async () => {
    ...
    const apiKey = import.meta.env.VITE_GEMINI_API_KEY;
    const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`;
    const conversationHistory = newMessages.slice(-6).map(m => `${m.sender === 'user' ? '粉絲' : selectedArtist.name}: ${m.text}`).join('\n');
              ...
              <>
                ...
                {spotifyToken ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <button onClick={copyTracklistToClipboard} className={`w-full py-3 rounded-lg font-bold transition-all flex justify-center items-center ${copySuccess ? 'bg-slate-700 text-white' : 'bg-slate-800 text-white hover:bg-slate-900 shadow-md'}`}>
                      {copySuccess ? '✅ 已複製！可前往 TuneMyMusic 貼上' : <><CopyIcon /> 複製歌單文字</>}
                    </button>
                  </div>
                )}
              </>
